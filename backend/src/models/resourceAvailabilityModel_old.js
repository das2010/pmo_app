...
module.exports = {
  // Obtener todas las disponibilidades de un recurso
  ...
  create: async ({ resource_id, from_date, to_date, daily_hours }) => {
    ...
    const hasOverlap = await this.checkOverlap(resource_id, from_date, to_date, null);
    if (hasOverlap) {
  ...
  update: async (id, { resource_id, from_date, to_date, daily_hours }) => {
    // Validar si hay solapamiento
    const hasOverlap = await this.checkOverlap(resource_id, from_date, to_date, id);
    if (hasOverlap) {
      throw new Error('El rango de fechas se solapa con otro período existente.');
    }
  ...
  // Si 'excludeId' existe, se ignora esa fila en la comprobación (caso update).
  checkOverlap: async function(resourceId, fromDate, toDate, excludeId) {
    let query = `
      SELECT *
      FROM resource_availabilities
      WHERE resource_id = $1
        AND (
          -- Se superpone si "fromDate" está dentro de un rango existente
          (from_date <= $2 AND to_date >= $2)
          -- O si "toDate" está dentro de un rango existente
          OR (from_date <= $3 AND to_date >= $3)
          -- O si el nuevo rango contiene totalmente a un rango existente
